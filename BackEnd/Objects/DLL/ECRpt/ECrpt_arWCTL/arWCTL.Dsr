VERSION 5.00
Begin {82282820-C017-11D0-A87C-00A0C90F29FC} arWCTL 
   Caption         =   "Weekly Claim Time Log"
   ClientHeight    =   11115
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   15240
   ...
   _ExtentX        =   26882
   _ExtentY        =   19606
   SectionData     =   "arWCTL.dsx":0000
End
Attribute VB_Name = "arWCTL"
...
Private mcolWCTL01 As Collection
Private mlWCTLCount As Long
...
Private Sub ActiveReport_ReportStart()
    ...
    Dim lErrNum As Long
    Dim sErrDesc As String
    Dim sDecTemp As String
    Dim MyFromDate As Date
    ...
    For Each oField In Me.ReportHeader.Controls
         If GetProperty(CStr(oField.Name)) <> vbNullString Then
            If oField.Tag = "DecFormat" Then
                sDecTemp = GetProperty(CStr(oField.Name))
                sDecTemp = Format(sDecTemp, "0.00")
                oField.Text = sDecTemp
            Else
    ...
    'Populate the days of week for the Page Header
    MyFromDate = GetProperty("f_FromDate")
    f_SunDayNo.Text = Format(MyFromDate, "dd")
    f_MonDayNo.Text = Format(DateAdd("d", 1, MyFromDate), "dd")
    f_TueDayNo.Text = Format(DateAdd("d", 2, MyFromDate), "dd")
    f_WedDayNo.Text = Format(DateAdd("d", 3, MyFromDate), "dd")
    f_ThuDayNo.Text = Format(DateAdd("d", 4, MyFromDate), "dd")
    f_FriDayNo.Text = Format(DateAdd("d", 5, MyFromDate), "dd")
    f_SatDayNo.Text = Format(DateAdd("d", 6, MyFromDate), "dd")
    ...
    Set mcolWCTL01 = mcolProperty("coludtWCTL01")
    mlWCTLCount = 1
    If Not mcolWCTL01 Is Nothing Then
        mlMaxCount = mcolWCTL01.Count
    End If
...
Private Sub Detail_Format()
    ...
    If mlWCTLCount <= mlMaxCount Then
        MyWCTL = mcolWCTL01(mlWCTLCount)
        With MyWCTL
            f_WCTLCount.Text = Format(mlWCTLCount, "0.")
            f_ClaimNumber.Text = .f_ClaimNumber
            f_Status.Text = .f_Status
            f_Sun.Text = Format(.f_Sun, "0.00")
            f_Mon.Text = Format(.f_Mon, "0.00")
            f_Tue.Text = Format(.f_Tue, "0.00")
            f_Wed.Text = Format(.f_Wed, "0.00")
            f_Thu.Text = Format(.f_Thu, "0.00")
            f_Fri.Text = Format(.f_Fri, "0.00")
            f_Sat.Text = Format(.f_Sat, "0.00")
            f_ClaimTtl.Text = Format(.f_ClaimTtl, "0.00")
        End With
        Detail.PrintSection
        mlWCTLCount = mlWCTLCount + 1
    Else
        If Not moChainReport Is Nothing Then
...
Public Function GetXMLExport() As String
    ...
    'Weekly Claim time Log Report Needs to include Collection of Entries
    Dim MyWCTL As udtWCTL01
    Dim oWCTLRS As WDDXRecordset
    ...
    For lCount = 1 To mcolPropertyKeys.Count
        ...
        If StrComp(sColName, "coludtWCTL01", vbTextCompare) <> 0 Then
    ...
    '****BEGIN***Weekly Claim time Log Report Needs to include Collection of Entries***
    Set mcolWCTL01 = mcolProperty("coludtWCTL01")
    If Not mcolWCTL01 Is Nothing Then
        If mcolWCTL01.Count > 0 Then
            Set oWCTLRS = New WDDXRecordset
            'Add Colmn names
            oWCTLRS.addColumn "AssignmentsID"
            oWCTLRS.addColumn "f_ClaimNumber"
            oWCTLRS.addColumn "f_ClaimTtl"
            oWCTLRS.addColumn "f_Fri"
            oWCTLRS.addColumn "f_Mon"
            oWCTLRS.addColumn "f_Sat"
            oWCTLRS.addColumn "f_Status"
            oWCTLRS.addColumn "f_Sun"
            oWCTLRS.addColumn "f_Thu"
            oWCTLRS.addColumn "f_Tue"
            oWCTLRS.addColumn "f_Wed"
            'Add the same number in collection
            oWCTLRS.addRows mcolWCTL01.Count
        End If
    
    End If
         
    For lCount = 1 To mcolWCTL01.Count
        MyWCTL = mcolWCTL01(lCount)
        With MyWCTL
            vValue = .AssignmentsID
            oWCTLRS.setField lCount, "AssignmentsID", vValue
            vValue = .f_ClaimNumber
            oWCTLRS.setField lCount, "f_ClaimNumber", vValue
            vValue = .f_ClaimTtl
            oWCTLRS.setField lCount, "f_ClaimTtl", vValue
            vValue = .f_Fri
            oWCTLRS.setField lCount, "f_Fri", vValue
            vValue = .f_Mon
            oWCTLRS.setField lCount, "f_Mon", vValue
            vValue = .f_Sat
            oWCTLRS.setField lCount, "f_Sat", vValue
            vValue = .f_Status
            oWCTLRS.setField lCount, "f_Status", vValue
            vValue = .f_Sun
            oWCTLRS.setField lCount, "f_Sun", vValue
            vValue = .f_Thu
            oWCTLRS.setField lCount, "f_Thu", vValue
            vValue = .f_Tue
            oWCTLRS.setField lCount, "f_Tue", vValue
            vValue = .f_Wed
            oWCTLRS.setField lCount, "f_Wed", vValue
        End With
    Next
    '****END***Weekly Claim time Log Report Needs to include Collection of Entries***
    ...
    If Not oWCTLRS Is Nothing Then
        oMyStruct.setProp "WCTLRS", oWCTLRS
    End If
