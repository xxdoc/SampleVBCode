VERSION 5.00
Begin {82282820-C017-11D0-A87C-00A0C90F29FC} arUsersInfo 
   ...
   _ExtentX        =   26882
   _ExtentY        =   19606
   SectionData     =   "arUsersInfo.dsx":0000
End
Attribute VB_Name = "arUsersInfo"
...
Private mcolUsers01 As Collection
...
Private Sub ActiveReport_ReportStart()
    ...
    For Each oField In Me.ReportFooter.Controls
        If GetProperty(CStr(oField.Name)) <> vbNullString Then
            If oField.Tag = "PercentFormat" Then
    ...
    Set mcolUsers01 = mcolProperty("coludtUsers01")
    mlUserCount = 1
    If Not mcolUsers01 Is Nothing Then
        mlMaxCount = mcolUsers01.Count
    End If
...
Private Sub Detail_Format()
    ...
    Dim lLenOfHiddenPass As Long
    
    If mlUserCount <= mlMaxCount Then
        MyUsers = mcolUsers01(mlUserCount)
        With MyUsers
                f_UserCount.Text = Format(mlUserCount, "0.")
                f_ContactPhone.Text = .f_ContactPhone
                f_DateLastUpdated.Text = .f_DateLastUpdated
                f_Email.Text = .f_Email
                f_FirstName.Text = .f_FirstName
                f_LastName.Text = .f_LastName
                'Only Decode the password if Security level
                'is =1 Basic
                'And do't decode "CFUSER" or "UUNASSIGNED"
                If .SecurityLevel = 1 Then
                    If StrComp("CFUSER", .f_UserName, vbTextCompare) = 0 Then
                        lLenOfHiddenPass = Len(goUtil.Decode(.f_PassWord))
                    ElseIf StrComp("UUNASSIGNED", .f_UserName, vbTextCompare) = 0 Then
                        lLenOfHiddenPass = Len(goUtil.Decode(.f_PassWord))
                    ElseIf StrComp("ZVOIDCLAIMS", .f_UserName, vbTextCompare) = 0 Then
                        lLenOfHiddenPass = Len(goUtil.Decode(.f_PassWord))
                    Else
                        lLenOfHiddenPass = 0
                    End If
                Else
                    lLenOfHiddenPass = Len(goUtil.Decode(.f_PassWord))
                End If
                If lLenOfHiddenPass = 0 Then
                    f_PassWord.Text = goUtil.Decode(.f_PassWord)
                Else
                    f_PassWord.Text = String(lLenOfHiddenPass, "*")
                End If
                f_UserName.Text = .f_UserName
        End With
        Detail.PrintSection
        mlUserCount = mlUserCount + 1
    Else
        If Not moChainReport Is Nothing Then
...
Public Function GetXMLExport() As String
    ...
    'UsersInfo Report Needs to include Collection of Entries
    Dim MyUsers As udtUsers01
    Dim oUsersRS As WDDXRecordset
    ...
    For lCount = 1 To mcolPropertyKeys.Count
        ...
        If StrComp(sColName, "coludtUsers01", vbTextCompare) <> 0 Then
    ...
    '****BEGIN***UsersInfo Report Needs to include Collection of Entries***
    Set mcolUsers01 = mcolProperty("coludtUsers01")
    If Not mcolUsers01 Is Nothing Then
        If mcolUsers01.Count > 0 Then
            Set oUsersRS = New WDDXRecordset
            'Add Colmn names
            oUsersRS.addColumn "UsersID"
            oUsersRS.addColumn "f_UserName"
            oUsersRS.addColumn "f_PassWord"
            oUsersRS.addColumn "f_FirstName"
            oUsersRS.addColumn "f_LastName"
            oUsersRS.addColumn "f_Email"
            oUsersRS.addColumn "f_ContactPhone"
            oUsersRS.addColumn "f_DateLastUpdated"
            'Add the same number in collection
            oUsersRS.addRows mcolUsers01.Count
        End If
    
    End If
         
    For lCount = 1 To mcolUsers01.Count
        MyUsers = mcolUsers01(lCount)
        With MyUsers
            vValue = .UsersID
            oUsersRS.setField lCount, "UsersID", vValue
            vValue = .f_UserName
            oUsersRS.setField lCount, "f_UserName", vValue
            vValue = .f_PassWord
            oUsersRS.setField lCount, "f_PassWord", vValue
            vValue = .f_FirstName
            oUsersRS.setField lCount, "f_FirstName", vValue
            vValue = .f_LastName
            oUsersRS.setField lCount, "f_LastName", vValue
            vValue = .f_Email
            oUsersRS.setField lCount, "f_Email", vValue
            vValue = .f_ContactPhone
            oUsersRS.setField lCount, "f_ContactPhone", vValue
            vValue = .f_DateLastUpdated
            oUsersRS.setField lCount, "f_DateLastUpdated", vValue
        End With
    Next
    '****END***UsersInfo Report Needs to include Collection of Entries***
    ...
    If Not oUsersRS Is Nothing Then
        oMyStruct.setProp "UsersRS", oUsersRS
    End If
