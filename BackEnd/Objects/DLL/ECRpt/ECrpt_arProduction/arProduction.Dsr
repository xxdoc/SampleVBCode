VERSION 5.00
Begin {82282820-C017-11D0-A87C-00A0C90F29FC} arProduction 
   ...
   _ExtentX        =   26882
   _ExtentY        =   19606
   SectionData     =   "arProduction.dsx":0000
End
Attribute VB_Name = "arProduction"
...
Private mcolClaims01 As Collection
Private mlClaimsCount As Long
...
Private Sub ActiveReport_ReportStart()
    ...
    Set mcolClaims01 = mcolProperty("coludtClaims01")
    mlClaimsCount = 1
    If Not mcolClaims01 Is Nothing Then
        mlMaxCount = mcolClaims01.Count
    End If
...
Private Sub Detail_Format()
    ...
    If mlClaimsCount <= mlMaxCount Then
        MyClaims = mcolClaims01(mlClaimsCount)
        With MyClaims
                f_DaysInspToClose.Text = .f_DaysInspToClose
                f_ClaimsCount.Text = Format(mlClaimsCount, "0.")
                f_CLIENTNUM.Text = .f_CLIENTNUM
                f_Insured.Text = .f_Insured
                f_CloseDate.Text = IIf(.f_CloseDate = NULL_DATE, vbNullString, .f_CloseDate)
                f_ContactDate.Text = IIf(.f_ContactDate = NULL_DATE, vbNullString, .f_ContactDate)
                f_InspectedDate.Text = IIf(.f_InspectedDate = NULL_DATE, vbNullString, .f_InspectedDate)
                f_ReceivedDate.Text = IIf(.f_ReceivedDate = NULL_DATE, vbNullString, .f_ReceivedDate)
        End With
        Detail.PrintSection
        mlClaimsCount = mlClaimsCount + 1
    Else
        If Not moChainReport Is Nothing Then
...
Public Function GetXMLExport() As String
    ...
    'Production Report Needs to include Collection of Entries
    Dim MyClaims As udtClaims01
    Dim oClaimsRS As WDDXRecordset
    ...
    For lCount = 1 To mcolPropertyKeys.Count
        ...
        If StrComp(sColName, "coludtClaims01", vbTextCompare) <> 0 Then
    ...
    '****BEGIN***Production Report Needs to include Collection of Entries***
    Set mcolClaims01 = mcolProperty("coludtClaims01")
    If Not mcolClaims01 Is Nothing Then
        If mcolClaims01.Count > 0 Then
            Set oClaimsRS = New WDDXRecordset
            'Add Colmn names
            oClaimsRS.addColumn "AssignmentsID"
            oClaimsRS.addColumn "f_CLIENTNUM"
            oClaimsRS.addColumn "f_CloseDate"
            oClaimsRS.addColumn "f_CloseDate"
            oClaimsRS.addColumn "f_ContactDate"
            oClaimsRS.addColumn "f_DaysInspToClose"
            oClaimsRS.addColumn "f_InspectedDate"
            oClaimsRS.addColumn "f_Insured"
            oClaimsRS.addColumn "f_ReceivedDate"
            'Add the same number in collection
            oClaimsRS.addRows mcolClaims01.Count
        End If
    
    End If
         
    For lCount = 1 To mcolClaims01.Count
        MyClaims = mcolClaims01(lCount)
        With MyClaims
            vValue = .AssignmentsID
            oClaimsRS.setField lCount, "AssignmentsID", vValue
            vValue = .f_CLIENTNUM
            oClaimsRS.setField lCount, "f_CLIENTNUM", vValue
            vValue = .f_CloseDate
            oClaimsRS.setField lCount, "f_CloseDate", vValue
            vValue = .f_CloseDate
            oClaimsRS.setField lCount, "f_CloseDate", vValue
            vValue = .f_ContactDate
            oClaimsRS.setField lCount, "f_ContactDate", vValue
            vValue = .f_DaysInspToClose
            oClaimsRS.setField lCount, "f_DaysInspToClose", vValue
            vValue = .f_InspectedDate
            oClaimsRS.setField lCount, "f_InspectedDate", vValue
            vValue = .f_Insured
            oClaimsRS.setField lCount, "f_Insured", vValue
            vValue = .f_ReceivedDate
            oClaimsRS.setField lCount, "f_ReceivedDate", vValue
        End With
    Next
    '****END***Production Report Needs to include Collection of Entries***
    ...
    If Not oClaimsRS Is Nothing Then
        oMyStruct.setProp "ClaimsRS", oClaimsRS
    End If
