VERSION 5.00
Begin {82282820-C017-11D0-A87C-00A0C90F29FC} arWorkSheetDiag 
   ...
   _ExtentX        =   26882
   _ExtentY        =   19606
   SectionData     =   "arWorkSheetDiag.dsx":0000
End
Attribute VB_Name = "arWorkSheetDiag"
...
Private Sub ActiveReport_ReportStart()
    ...
    For Each oField In Me.Detail.Controls
        If TypeOf oField Is DDActiveReports.Field Then
            ...
            sTemp = GetProperty(CStr(sFieldName))
            If InStr(1, oField.Tag, "vbDate", vbTextCompare) > 0 Then
                If IsDate(sTemp) And sTemp <> NULL_DATE Then
                    oField.Text = sTemp
                Else
        ...
        ElseIf TypeOf oField Is DDActiveReports.Image Then
            'Clear memory !
            sFieldName = oField.Name
            If sFieldName = "img_DiagramPhotoName" Then
                oField.Picture = LoadPicture()
                sImagePath = GetProperty(sFieldName)
                If goUtil.utFileExists(sImagePath) Then
                    oField.Picture = LoadPicture(sImagePath)
                End If
            End If
        End If
    Next
...
'    End If
    'Clear Memory
    img_DiagramPhotoName.Picture = LoadPicture()
