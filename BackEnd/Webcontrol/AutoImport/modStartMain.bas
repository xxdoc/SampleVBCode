...
Public Sub Main()
    ...
    SaveSetting "AutoImport", "Msg", "Reset", False
    'If we already have this running then Bail
    If App.PrevInstance Then
        goUtil.utDeleteFile App.Path & "\" & App.EXEName & ".bat"
        goUtil.CLEANUP
        Set goUtil = Nothing
        End
        Exit Sub
    Else
        goUtil.utDeleteFile App.Path & "\" & App.EXEName & ".bat"
        If Command$ = "RunAsDepOfV2WebControlService" Then
            GoTo RUN_SERVICE
        Else
            goUtil.CLEANUP
            Set goUtil = Nothing
            MsgBox Command$ & vbCrLf & vbCrLf & App.EXEName & " may only be run as a dependant of the Service WebControl.", vbExclamation
            End
            Exit Sub
        End If
    End If
